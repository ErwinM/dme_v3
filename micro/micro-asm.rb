...
class Parser
  ...
  def Parser.checkword(word)
    if !(["sext", "re", "wpte", "wptb", "brk", "ureg", "reti", "syscall", "incr_sp", "skipstate","alu", "mar_load", "ir_load", "mdr_load", "reg_load", "ram_load", "incr_pc", "decr_sp", "be", "mdrs", "regr0s", "regr1s", "regws", "imms", "op0s", "op1s", "skipc"]).include?(word.strip.downcase) then
  ...
  def self.encode(output, intelhex, memlist)
    ["DECODE", "READ", "EXEC"].each do |cycle|
      ...
      $instructions.each do |line, instr|
        ...
        instr["SEXT"] ?  microcode += "1" : microcode +="0"
